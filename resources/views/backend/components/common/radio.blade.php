    ...
    <div {{$attributes->class(['d-flex align-items-center gap-3 mb-4'])}}>
        @foreach($options as $key => $option)
            <div class="form-check form-check-success">
                {{-- <input class="form-check-input" type="radio" name="{{$name}}" value="{{gv($option, 'id')}}" id="flexRadioSuccess_{{$name}}_{{$key}}" {{ old($name) == gv($option, 'id') ? 'checked' : ($value ? ($value == gv($option, 'id') ? 'checked' : '') : (gv($option, 'checked') ? 'checked' : ''  )) }}> --}}
